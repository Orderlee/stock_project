...
class StockDao(StockDto):
    ...
    def find_all_by_ticker(cls, stock):
        sql = cls.query
        df = pd.read_sql(sql.statement, sql.session.bind)
        # df = session.query(cls).all()
        print(df)
        df = df[df[6] == stock.ticker]
        #return session.query(cls).filter(cls.ticker == stock.ticker).all()
        return json.loads(df.to_json(orient='records'))
...
class Stock(Resource):
    ...
    def post(self):
        ...
        stock = StockDto(data['date'], data['ticker'],data['open'], data['high'], data['low'], data['close'], data['volume'])  
        try: 
            stock.save(data)
...
class lgchem(Resource):
    ...
    def post():
        ...
        data = StockDao.find_all_by_ticker(stock)
        print('find_all_by_ticker success')
        return data[0], 200
